...
GT(n, r) =
\begin{cases}
\text{go-alone}(n, r) & \text{if } n \text{ is a Truth Zone,} \\
\max\left( \text{go-alone}(n, r),\ \displaystyle\max_{m,\, r'}\ p(n, m, r') \cdot GT(m, r') \right) & \text{otherwise.}
\end{cases}
$$
...
├── data/                    # All datasets are git-ignored — download them (see Data)
│   ├── metrica/             # Metrica sample matches (events + tracking) — download separately
│   ├── statsbomb_360/       # StatsBomb 360 JSON — download separately
│   └── processed/           # Datasets generated by the scripts (regenerable)
...
without retraining. **No external dataset is committed to this repository** — the data belong to
their respective providers and are only *referenced* here. Download them from the original sources
below (both are publicly available for research/education; please respect their licenses):
...
- **Metrica Sports Sample Data** (event + tracking data used to *evaluate* GT) —
  <https://github.com/metrica-sports/sample-data>. Copy the game folders under `data/metrica/`,
  keeping the original file names, e.g.:
  - `data/metrica/Game_2_Metrica/Sample_Game_2_RawEventsData.csv`
  - `data/metrica/Game_2_Metrica/Sample_Game_2_RawTrackingData_Home_Team.csv`
  - `data/metrica/Game_2_Metrica/Sample_Game_2_RawTrackingData_Away_Team.csv`

Once the raw data are in place, the `data/processed/` datasets are (re)generated by the scripts in
[Usage](#usage) below.
